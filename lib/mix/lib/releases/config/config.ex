          ...
          _ ->
            {:ok,
             %{
               ...
               | :environments =>
                   Enum.into(
                     Enum.map(base_config.environments, fn {name, e} ->
                       {name,
                        %{
                          e
                          | :profile => %{
                              ...
                              | :dev_mode => get_opt(opts, :dev_mode, e.profile.dev_mode),
                                ...
                                :exec_opts =>
                                  Enum.into(get_opt(opts, :exec_opts, e.profile.exec_opts), %{})
                            }
                        }}
                     end),
                     %{}
    ...
    for profile <- profiles do
      ...
      cond do
        ...
        not is_nil(profile.pre_configure_hook) and not is_binary(profile.pre_configure_hook) ->
          raise ArgumentError,
                "expected :pre_configure_hook to be nil or a path string, but got: #{
                  inspect(profile.pre_configure_hook)
                }"

        not is_nil(profile.post_configure_hook) and not is_binary(profile.post_configure_hook) ->
          raise ArgumentError,
                "expected :post_configure_hook to be nil or a path string, but got: #{
                  inspect(profile.post_configure_hook)
                }"

        not is_nil(profile.pre_start_hook) and not is_binary(profile.pre_start_hook) ->
          raise ArgumentError,
                "expected :pre_start_hook to be nil or a path string, but got: #{
                  inspect(profile.pre_start_hook)
                }"

        not is_nil(profile.post_start_hook) and not is_binary(profile.post_start_hook) ->
          raise ArgumentError,
                "expected :post_start_hook to be nil or a path string, but got: #{
                  inspect(config.post_start_hook)
                }"

        not is_nil(profile.pre_stop_hook) and not is_binary(profile.pre_stop_hook) ->
          raise ArgumentError,
                "expected :pre_stop_hook to be nil or a path string, but got: #{
                  inspect(profile.pre_stop_hook)
                }"

        not is_nil(profile.post_stop_hook) and not is_binary(profile.post_stop_hook) ->
          raise ArgumentError,
                "expected :post_stop_hook to be nil or a path string, but got: #{
                  inspect(profile.post_stop_hook)
                }"

        not is_nil(profile.pre_upgrade_hook) and not is_binary(profile.pre_upgrade_hook) ->
          raise ArgumentError,
                "expected :pre_upgrade_hook to be nil or a path string, but got: #{
                  inspect(profile.pre_upgrade_hook)
                }"

        not is_nil(profile.post_upgrade_hook) and not is_binary(profile.post_upgrade_hook) ->
          raise ArgumentError,
                "expected :post_upgrade_hook to be nil or a path string, but got: #{
                  inspect(profile.post_upgrade_hook)
                }"
