defmodule Mix.Releases.Assembler do
  ...
  **Note: This operation has side-effects!** It creates files, directories,
  ...
  defp create_release_info(
         %Release{name: relname, profile: %Profile{output_dir: output_dir}} = release
       ) do
    ...
    case File.mkdir_p(rel_dir) do
      ...
      :ok ->
        ...
        start_clean_rel = %{
          release
          | :applications =>
              Enum.filter(release.applications, fn %App{name: n} ->
  ...
  defp generate_relup(
         %Release{name: name, upgrade_from: upfrom, profile: %Profile{output_dir: output_dir}} =
           release,
         rel_dir
       ) do
  ...
  defp generate_start_erl_data(
         %Release{version: version, profile: %Profile{include_erts: false}},
         rel_dir
       ) do
  ...
  defp generate_sys_config(
         %Release{profile: %Profile{config: base_config_path, sys_config: config_path}} = rel,
         rel_dir
       )
  ...
  defp generate_sys_config(
         %Release{profile: %Profile{config: config_path, included_configs: included_configs}},
         rel_dir
       ) do
    Logger.debug("Generating sys.config from #{Path.relative_to_cwd(config_path)}")
  ...
  defp include_erts(
         %Release{profile: %Profile{include_erts: include_erts, output_dir: output_dir}} = release
       ) do
    prefix =
      case include_erts do
        true ->
  ...
  defp apply_overlays(%Release{} = release) do
    ...
    hook_overlays =
      [
        ...
        {:copy, release.profile.pre_configure_hook,
         "#{hooks_dir}/pre_configure.d/00_pre_configure_hook.sh"},
        {:copy, release.profile.post_configure_hook,
         "#{hooks_dir}/post_configure.d/00_post_configure_hook.sh"},
        {:copy, release.profile.pre_start_hook, "#{hooks_dir}/pre_start.d/00_pre_start_hook.sh"},
        {:copy, release.profile.post_start_hook,
         "#{hooks_dir}/post_start.d/00_post_start_hook.sh"},
        ...
        {:copy, release.profile.pre_upgrade_hook,
         "#{hooks_dir}/pre_upgrade.d/00_pre_upgrade_hook.sh"},
        {:copy, release.profile.post_upgrade_hook,
         "#{hooks_dir}/post_upgrade.d/00_post_upgrade_hook.sh"},
