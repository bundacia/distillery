defmodule Mix.Releases.Archiver do
                 ...
                 false ->
                   case release.profile.include_system_libs do
                     false ->
                       ...
                       libs =
                         Enum.map(
                           Path.wildcard(Path.join([tmpdir, "lib", "*"])),
                           &Path.basename/1
                         )

                       system_libs =
                         Enum.map(
                           Path.wildcard(Path.join("#{:code.lib_dir()}", "*")),
                           &Path.basename/1
                         )

                       for libdir <- :lists.subtract(libs, system_libs),
                           do:
                             {'#{Path.join("lib", libdir)}',
                              '#{Path.join([tmpdir, "lib", libdir])}'}
  ...
  defp strip_release(
         %Release{is_upgrade: false, profile: %Profile{strip_debug_info: true, dev_mode: false}},
         strip_path
       ) do
    Logger.warn(
  ...
  defp strip_release(
         %Release{is_upgrade: true, profile: %Profile{strip_debug_info: true, dev_mode: false}},
         _strip_path
       ) do
    Logger.warn(
  ...
  defp strip_release(
         %Release{profile: %Profile{strip_debug_info: true, dev_mode: true}},
         _strip_path
       ) do
    Logger.warn(
